...
loadDictionaries <- function() {
  # Seed from bundled package data
  load(system.file("data", "current.rda", package = "alspac"), envir = globals)

  # Override with user-cached versions if updateDictionaries() has been run
...
retrieveDictionary <- function(name) {
  ...
  # Check user cache
  cache_dir <- tools::R_user_dir("alspac", "cache")
  f <- file.path(cache_dir, paste0(name, ".rdata"))
  if (file.exists(f)) {
    load(f, envir = globals)
    return(get(name, envir = globals))
  }
  ...
  for (dict_name in dict_names) {
    ...
    for (col in columns_to_check) {
      ...
      if (col %in% colnames(dict)) {
        ...
        if (num_NA > 0) {
          message(sprintf("Dictionary '%s' has %d missing values in column '%s'.", dict_name, num_NA, col))
          missing_found <- TRUE
        }
      } else {
        message(sprintf("Column '%s' not found in dictionary '%s'.", col, dict_name))
      }
    }

    # If no missing values were found, print this message once per dictionary
    if (!missing_found) {
      message(sprintf("Dictionary '%s' has no missing values in the Withdrawal of Consent logic columns.", dict_name))
    }
  }
}
